		...
		for (const pkg of packages) {
			try {
				...
				changes.push({
					...
					needsUpdate,
					willCreateTag,
					tagName: willCreateTag
						? updater.getTagName(pkg)
						: undefined,
				});
			} catch (error) {
				this.observer.onError(
	...
	private async processPackage(change: PackageChange): Promise<ProcessResult> {
		...
		try {
			...
			if (change.willCreateTag) {
				...
				const tagResult = await this.gitOps.createTag(
					tagName,
					this.dryRun,
				);
			...
			this.observer.onError(`Failed to process ${config.name}: ${errorMessage}`);
			return {
				package: config,
				updated: false,
				skipped: false,
