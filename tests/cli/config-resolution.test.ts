...
`;
	...
	describe('Path filtering', () => {
		it('should filter packages by exact path match', () => {
			const packages = [
				{ type: 'npm', path: 'packages/service-a', name: 'service-a', version: '1.1.0' },
				{ type: 'npm', path: 'packages/service-b', name: 'service-b', version: '1.2.0' },
				{ type: 'npm', path: 'tools/cli', name: 'cli-tool', version: '2.0.0' },
			];

			const filtered = packages.filter(pkg => pkg.path === 'packages/service-a');
		...
		it('should filter packages by path prefix', () => {
			...
			const filtered = packages.filter(pkg => pkg.path.startsWith('packages/'));
	...
	describe('Integration tests', () => {
		it('should process only packages in current subdirectory', async () => {
			...
			expect(filtered).toHaveLength(2);
			expect(filtered.map(p => p.name).sort()).toEqual(['service-a', 'service-b']);
		});
